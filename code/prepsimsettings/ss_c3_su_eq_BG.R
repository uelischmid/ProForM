## create simsettings for c3_su_eq_BG simulations
## 27.1.23, us
...
out_folder <- "data/processed/simsettings/c3_su/"
...
times <- tribble(
  ~stratum, ~quality, ~simtime, ~mgmtime,
      "UM",     "31",    1160L,    "160",
      "UM",     "33",    1075L,    "075",
      "UM",     "51",    1160L,    "160",
      "UM",     "55",    1060L,    "060"
...
combinations <- comb_str_q_init %>%
  left_join(times, by = c("stratum", "quality")) %>% 
  replace_na(list(simtime = 1000)) %>% 
...
for (i in 1:nrow(combinations)) {
  ...
  # new file name
  filename_new <- str_c("simsettings_c3_su_eq_BG_",
                        pull(vals, stratum), "_",
                        pull(vals, quality), ".csv")
  
  # sim name
  ss_new[[1, 2]] <- str_c("c3_su_eq_BG_",
                          pull(vals, stratum), "_",
                          pull(vals, quality))
  
  # sim_init
  ss_new[[2, 2]] <- str_c("c3_BG/init_",
                          pull(vals, stratum), "_BG_",
                          pull(vals, grid_dim), "-", pull(vals, grid_dim),
                          ".csv")
  ...
  # mgm
  if (pull(vals, stratum) == "UM") {
    ss_new[[18, 2]] <- "single_tree_selection"
    ss_new[[19, 2]] <- str_c("c3_su/mgm_c3_su_eq_BG_",
                             pull(vals, stratum), "_t",
                             pull(vals, mgmtime), ".csv")
  } else {
    ss_new[[18, 2]] <- "none"
    ss_new[[19, 2]] <- "NA"
  }
