...
for (i in seq_along(assessments)) {
  ...
  # assemble ----
  res[[i]] <- tibble(stratum       = assessment_infos[4],
                     quality       = assessment_infos[5],
                     q_site        = as.numeric(str_sub(assessment_infos[5], end = 1)),
                     q_reg         = as.numeric(str_sub(assessment_infos[5], start = 2)),
                     init          = str_sub(assessment_infos[6], start = -1),
                     mgm           = assessment_infos[7],
                     mgm_interval  = as.numeric(str_sub(assessment_infos[8], start = 2)),
                     mgm_intensity = as.numeric(str_sub(assessment_infos[9], start = 2)),
                     nat_haz       = assessment_infos[10],
