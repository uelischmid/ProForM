...
# setup -------------------------------------------------------------------
library(tidyverse)
split_vector <- function(x, n = 2) split(x, cut(seq_along(x), n, labels = FALSE))

nais_preps <- list.dirs("data/processed/naisoutput/1_prep", full.names = FALSE, recursive = FALSE)
nais_preps <- nais_preps[str_detect(nais_preps, "c3_LT")]
