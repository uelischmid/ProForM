...
analysis_relaxed <- read_rds("results/c3_analysis_ST/ST_relaxed.rds") %>% 
  mutate(temp_sim_ID = str_c(stratum, "_", quality, "_init", init, "_",
...
for (i in seq_along(vis_groups)) {
  ...
  analysis_values_NOM <- filter(analysis_relaxed,
                                temp_sim_ID == str_replace(ass_NOM, "c3_ST_def_", ""))
  analysis_values_mgm <- filter(analysis_relaxed,
                                temp_sim_ID == str_replace(ass_mgm, "c3_ST_def_", ""))
